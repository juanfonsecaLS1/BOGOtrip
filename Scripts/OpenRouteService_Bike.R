...
time_long<-as.data.frame(Duration_mat)%>%
  ...
  pivot_longer(cols = -O,names_to = "D",names_prefix = "V",values_to = "Bike_time")

dist_long<-as.data.frame(Distance_mat)%>%
  rownames_to_column(var = "O")%>%
  pivot_longer(cols = -O,names_to = "D",names_prefix = "V",values_to = "Bike_dist")


# Saves a CSV file with the matrices and the UPZ list
write_csv(left_join(time_long,dist_long,by=c("O","D")),file = "Bike_matrices.csv",quote_escape = FALSE)
write_csv(Zones%>%rownames_to_column("ID")%>%select(ID,UPlCodigo),file = "UPZ_ID.csv",quote_escape = FALSE)
